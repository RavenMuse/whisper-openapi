...
class CONFIG:
    ...
    if ASR_ENGINE == "whisperx" and HF_TOKEN == "":
        print(
            "You must set the HF_TOKEN environment variable to download the diarization model used by WhisperX."
        )
    ...
    MODEL_QUANTIZATION = os.getenv(
        "ASR_QUANTIZATION", "float32" if torch.cuda.is_available() else "int8"
    )
    if MODEL_QUANTIZATION not in {"float32", "float16", "int8"}:
        raise ValueError(
            "Invalid MODEL_QUANTIZATION. Choose 'float32', 'float16', or 'int8'."
        )
    ...
    SUBTITLE_HIGHLIGHT_WORDS = (
        os.getenv("SUBTITLE_HIGHLIGHT_WORDS", "false").lower() == "true"
    )
